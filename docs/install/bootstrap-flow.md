...
- `scripts/installs/installation.sh`: explicit manual installer; environment and step-order reference for AI supervision.
...
4. **AI + scripts — remaining setup.** Work directly in the main session; no delegation. Read workspace rules, index, relevant sources and domain guides. Retain bootstrap context; reconcile existing changes. Use `installation.sh` for environment initialization and dependency order, then run remaining scripts individually with their supported options. Scripts install dependencies and apply/link saved configuration. Follow script notices and domain guides for additional manual setup. Verify actual outcomes and required reloads after each step.
