...
- Suspends the `borders` process before the focus-based DFS scan so the active red border does not flash across every window, then relaunches `~/.config/borders/bordersrc` from the cleanup trap after success or failure. A same-workspace no-op leaves JankyBorders untouched.
