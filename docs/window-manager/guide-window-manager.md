...
- Suspends the `borders` process before the focus-based DFS scan so the active red border does not flash across every window, then relaunches `~/.config/borders/bordersrc` detached via `nohup` from the cleanup trap after success or failure. A same-workspace no-op leaves JankyBorders untouched.
- Bash 3.2-compatible and command-boundary mockable via `AEROSPACE_BIN` / `SKETCHYBAR_BIN` / `BORDERS_STOP_BIN` / `BORDERS_START_BIN`. Run `bash configs/aerospace/tests/swap-workspaces_test.sh` for deterministic state-machine coverage of no-op, border suspension/restart, argument validation, stable-temp refusal, fullscreen timing, flat, normal/rotated grid, unsupported-layout, and rollback paths.
