...
- ./configs/sketchybar/plugins/*.sh (24 files)
...
- Fullscreen suspension and all three movement legs (source → staging, target → source, staging → target) are one short-circuiting `aerospace eval` batch. Floating correction/root reset, DFS reordering, grid joins, fullscreen restoration, and final focus/mouse placement are likewise grouped into the smallest safe batches around the existing verification boundaries; rollback remains best-effort per captured window ID.
...
- Leaves JankyBorders running throughout the swap. AeroSpace publishes only the final focus from each eval refresh session, so the border should no longer chase every DFS snapshot selection.
- Bash 3.2-compatible and command-boundary mockable via `AEROSPACE_BIN` / `SKETCHYBAR_BIN`. The existing deterministic mock suite predates AeroSpace's `eval` command and must be extended before it can cover the batched implementation; validate the shortcut manually in AeroSpace meanwhile.
...
- Groups right-side items into brackets in visual order connectivity | resources | calendar: `calendar_group`, `resources`, `connectivity`
...
- Paints the spaces strip once at startup after all items/brackets exist
...
- Active item files: spaces.sh, calendar.sh, ram.sh, cpu.sh, battery.sh, vpn.sh (defines TWO items: vpn_be + vpn_sn), wifi.sh, ethernet.sh; volume.sh/headset.sh were deleted with the audio division
...
- Event handlers and data fetchers (24 files)
...
- aerospace.sh: workspace display with multi-monitor colors. Renders app ICONS via sketchybar-app-font (__icon_map in icon_map.sh) when EVERY app in a space is mapped, else falls back to text names (shorten_app_name). Unmapped apps return `:default:` from the icon map and retain their full display name in the text fallback. Subscribes front_app_switched so it repaints on app open, not only on workspace change
