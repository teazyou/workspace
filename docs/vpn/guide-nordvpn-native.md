...
| Bar item | `configs/sketchybar/items/vpn.sh` + `plugins/vpn.sh` + `plugins/vpn_click.sh` | two text items — **BE** (Belgium) and **SN** (Singapore, code `sg`) — on two channels. **Colour = the tunnel:** grey = not connected, red = connected, yellow = connecting, magenta = `nord refresh` needed. **A thin UNDERLINE below one label = the selected country** (`~/.config/nordvpn-native/country`), always in that label's colour, exactly one at a time (it is the element's icon slot — a 14 px `─` U+2500 hairline at `:Regular:13.0` — pulled under the label with a negative `icon.padding_right`, so it costs zero layout width; hidden via `icon.color=$TRANSPARENT`, geometry from theme.sh `SEL_UNDERLINE_*`). So grey+underline = "off, this is what a click dials"; red+underline = "connected via this one". Click = `vpn_click.sh`: un-underlined icon → `nord <cc>` (switch + on), underlined icon → `nord toggle`; paints that icon busy (yellow "…") instantly and ignores re-clicks on EITHER icon until the action ends (200 s stale-steal). ONE shared click lock `/tmp/nordvpn-native.click`, whose `owner` file names the clicked item so `plugins/vpn.sh` keeps the busy look on the right icon while the other keeps rendering its real state. |
...
- **Only BE/SN are represented on the bar**; the CLI still supports all 6 countries. If the selected country is fr/my/us/vn (or a config is started by hand outside `nord`), **both** bar icons render grey **and no underline is drawn at all** — the "nothing selected on the bar" look is the tell — and the real exit stays unrepresented: a known 2-icon limitation, not a bug.
...
All six countries connect via `nord <cc>`; off durable across a forced watcher run; toggle cycle; boot simulation (garbage boot-id + kickstart → reset to sg/enabled + auto-connect in ~7 s); Wi-Fi flap self-heal; bar states red/orange/grey + CC label (**superseded 2026-07-30: two-icon BE/SN bar, then the colour+underline scheme (dot first, thin rule after) — no orange remains; see the Bar item row and re-verify per that table**); single approval covers all 6 payloads.
