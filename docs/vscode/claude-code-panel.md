...
This is now a saved, idempotent script — no terminal paste, no opening Claude Code:

```bash
workspace-patch   # re-applies THIS + the Obsidian patch (alias → scripts/workspace-patch.sh)
# ...or just this one:
scripts/claude/patch-vscode-panel.sh
```

It auto-finds the current extension version, backs up both files (timestamped `.bak`),
applies every patch idempotently (safe to re-run), and prints an `OK`/`MISSING` line per
patch. Then reload VS Code: **Cmd+Shift+P → "Developer: Reload Window"** (a full window
reload is required for the `extension.js` patch; "Reload Webviews" alone only refreshes the
`index.css` visual tweaks).

The exact `perl` substitutions + appended rules now live in the script at
[`scripts/claude/patch-vscode-panel.sh`](../../scripts/claude/patch-vscode-panel.sh) (kept
in sync with this guide); the per-patch reference is in the tables below.
