...
- `configs/sketchybar/items/*.sh` — one bar item definition per file (spaces, battery, wifi, etc.); paddings come from theme.sh tokens. Several items are state-driven (ethernet/headset show only when connected, bandwidth shows only the active direction, volume mutes, vpn=NordVPN glyph, wifi=RSSI bars).
- `configs/sketchybar/plugins/*.sh` — update/event scripts backing the items; `icon_map.sh` maps apps to sketchybar-app-font glyphs (used by `aerospace.sh` for the spaces app icons), `wifi.sh` does RSSI strength + auto-reconnect, `network_speed.sh` does VPN-tunnel-safe conditional bandwidth.
