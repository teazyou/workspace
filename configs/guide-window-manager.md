...
- `pollMillis=50` + `delay=1` + `requireMouseStop=true`: the cursor is focused as soon as it comes to a deliberate rest on a window (`delay=1` = no dwell; focus on the next ~50ms poll), while a quick flick up to the menu bar / SketchyBar doesn't steal focus from the windows it crosses (`requireMouseStop` ignores anything you don't actually stop on). `pollMillis` is only the responsiveness/stop-detection knob (a high value just adds latency — it was 200 and felt sluggish); the deliberate feel comes from `requireMouseStop`, not a slow poll. `delay>1` adds a hover-dwell (2 ≈ 50ms, 3 ≈ 100ms); `delay=0` disables raising
