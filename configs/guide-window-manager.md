...
- ON: unloads display-profile LaunchAgent, writes the state file FIRST, hides cpu + ram (battery STAYS) and stops the traffic poller (network_down drawing=off + update_freq=0), then runs network_speed.sh once — which (seeing state=on) hides both traffic divisions itself. KEEPS the volume/audio and connectivity groups visible. (JankyBorders left running.)
- OFF: reloads the display-profile LaunchAgent, clears the state file FIRST, re-enables cpu + ram and resumes the traffic poller (network_down drawing=on + update_freq=5), drops the stale prev_bytes cache (so the first recompute reports a real near-zero delta, not a giant spike from the frozen perf-mode gap), then runs network_speed.sh once to recompute real traffic visibility. Does NOT touch the traffic items/brackets/spacers directly
