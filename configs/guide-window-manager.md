...
- OFF: reloads the display-profile LaunchAgent, re-enables the always-visible resource items (cpu + ram) and the kept spacers (spacer0/1/2). Traffic is NOT blindly re-enabled — only network_down (the sole poller) is turned back on; network_up + both traffic brackets + the traffic spacers (spacer3/spacer_ud) start hidden and their visibility is recomputed by network_speed.sh, which is run once immediately. The stale prev_bytes cache is dropped first so the first recompute reports a real (near-zero) delta, not a giant spike from the frozen perf-mode gap
- Why OFF doesn't force the traffic brackets on: a `traffic_*` bracket renders a pill around its member item, so turning the bracket on while network_down's icon/label are still frozen `off` (from when that direction was last idle) drew an EMPTY pill until a later poll corrected it — the "up/down one segment empty" bug. Letting the poller set the conditional traffic visibility from a clean baseline avoids it
...
- network_speed.sh: single-poller (network_down). Up and down are SEPARATE divisions (brackets traffic_up / traffic_down with spacer_ud between), each shown only when its direction's rate clears MIN_RATE (~5 KB/s, to ignore the constant background trickle so idle directions disappear; spacer_ud shows only when both do; spacer3 = gap to connectivity). Parses netstat byte counters FROM THE RIGHT (Ibytes=NF-4, Obytes=NF-1) so VPN/tunnel interfaces — which drop the Address column — read correctly
