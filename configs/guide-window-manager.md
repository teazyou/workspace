...
- `pollMillis=50` + `delay=3` + `requireMouseStop=true`: the cursor must come to a deliberate rest on a window and is then focused ~100ms later (`delay=3` ≈ 100ms hover-dwell at the 50ms poll rate), so a quick flick up to the menu bar / SketchyBar doesn't steal focus from the windows it crosses. `pollMillis` is only the responsiveness/stop-detection knob (a high value just adds latency, it was 200 and felt sluggish); the deliberate feel comes from `requireMouseStop` + the dwell. `delay=0` disables raising
