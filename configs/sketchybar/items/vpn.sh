...
# grey=off, ORANGE=pinned server dead -> `nord refresh`. Click toggles the VPN
# (scripts/vpn/nord.sh toggle). Paddings from theme.sh. Custom `vpn_change` event is
# triggered by nord.sh/nord-connect.sh after every state change.
vpn=(
  icon=":nord_vpn:"
  ...
  padding_left=0
  padding_right=0
  click_script="bash $HOME/workspace/scripts/vpn/nord.sh toggle"
